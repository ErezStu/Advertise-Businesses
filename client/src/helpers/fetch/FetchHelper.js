...
export function getAllCards(token, callback) {
  let url = baseUrl + "/api/users/allcards";
  fetch(url, { headers: { "x-auth-token": token } })
    .then((x) => x.json())
    .then((x) => callback(x))
    .catch((x) => callback(x));
...
export function editCard(idToedit, token, callback) {
  let url = baseUrl + "/api/cards/" + idToedit;
  fetch(url, { method: "PATCH", headers: { "x-auth-token": token } })
    .then((x) => x.json())
    .then((x) => callback(x))
    .catch((x) => callback(x));
}
